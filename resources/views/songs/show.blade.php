    ...
    <div class="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-8 lg:grid-cols-3">
            ...
            <div class="lg:col-span-1">
                ...
                @if ($nextSong)
                    <a href="{{ $nextSongUrl }}" class="mt-3 flex gap-3 rounded-lg p-2 hover:bg-white/5">
                        <img
                            src="https://img.youtube.com/vi/{{ $nextSong->youtube_video_id }}/hqdefault.jpg"
                            alt=""
                            class="h-20 w-36 shrink-0 rounded object-cover bg-neutral-800"
                        >
                        <div class="min-w-0">
                            <p class="truncate text-sm font-medium text-white">{{ $nextSong->title }}</p>
                            <p class="truncate text-sm text-neutral-400">{{ $nextSong->artist->name }}</p>
                        </div>
                    </a>
                @else
