    ...
    >
        ...
        <div>
            ...
            <textarea id="bio" name="bio" rows="4"
                class="mt-1 w-full rounded-lg border border-black/10 bg-surface px-3 py-2 text-ink">{{ old('bio', $artist->bio) }}</textarea>
        ...
        <div class="flex gap-6">
            <label class="flex items-center gap-2 text-sm text-muted">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $artist->exists ? $artist->is_active : true)) class="rounded border-black/20 bg-surface">
                Active
            </label>

            <label class="flex items-center gap-2 text-sm text-muted">
                <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $artist->is_featured)) class="rounded border-black/20 bg-surface">
                Featured
            </label>
        </div>
