...
@auth
    @if (! $song->voting_enabled)
        <button type="button" disabled {{ $attributes->merge(['class' => 'inline-flex items-center justify-center rounded-lg bg-black/10 px-5 py-2.5 font-medium text-muted cursor-not-allowed']) }}>
            Voting Closed
        </button>
    @elseif ($hasVoted)
        <button type="button" disabled {{ $attributes->merge(['class' => 'inline-flex items-center justify-center rounded-lg bg-success/20 px-5 py-2.5 font-medium text-ink cursor-default']) }}>
            Voted Today
        </button>
    @else
