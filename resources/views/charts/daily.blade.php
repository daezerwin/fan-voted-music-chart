<x-layouts.app :title="'Daily Chart — '.$chart->chart_date->format('F j, Y')" description="The fan-voted daily Top chart.">
    <div class="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-ink">Daily Chart</h1>
            <span class="text-sm text-muted">{{ $chart->chart_date->format('F j, Y') }}</span>
        </div>

        <a href="{{ route('play') }}" class="mt-4 inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-ink hover:bg-primary/90">
            ▶ Play Top 10
        </a>
